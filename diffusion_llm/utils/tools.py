...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, feat_id) in tqdm(enumerate(vali_loader)):
            ...
            if args.use_amp:
                ...
                if args.output_attention:
                    ...
                    outputs, ddpm_loss = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_y=None, feat_id=feat_id)

            outputs, batch_y = accelerator.gather_for_metrics((outputs, batch_y))
            ddpm_loss = accelerator.gather_for_metrics(ddpm_loss)
    ...
    total_ddpm_loss = np.average(total_ddpm_loss)

    accelerator.print(f"Validation DDPM Loss: {total_ddpm_loss:.7f}")  # Log ddpm_loss
